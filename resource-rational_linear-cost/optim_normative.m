function optim_normative(mpr_index)
...
fun_LL_cost = @(pars) -sum(LL_costNEW_clust(mi,setsz,delta_s_col,response,N_samp,pars));

[params_fit , nll, ~, ~] = bads(fun_LL_cost,start_pars,LB,UB, PLB, PUB, OPTIONS);

[loglikk, Jbars_optim] = LL_costNEW_clust(mi,setsz,delta_s_col,response,N_samp,params_fit);


savefilename=['real_fits_fminbnd_norm_linear/', num2str(N_samp),'_samp/linear_real_norm_fits_fminbnd_',num2str(Nruns),'_starts_sbj_',num2str(par),'_run_',num2str(runi),'.mat'];
